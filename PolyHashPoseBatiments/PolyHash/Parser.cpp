    ...
    if (myfile.is_open())
    {
        ...
        while(getline(myfile,line)){
            ...
            switch(*sep[0].c_str()){
                case 'R':
                    Batiment *residence;
                    residence = new Residence(stoi(sep[3], &sz), stoi(sep[1], &sz), stoi(sep[2], &sz),mymap,CPT);
					CPT++;
                    //map->listeResidences.push_back(*residence);
					this->map->nbResidence++;
                    map->listeBatiments.push_back(residence);
                    break;
                case 'U':
                    Batiment *utility = new Utility(stoi(sep[3],&sz),stoi(sep[1],&sz),stoi(sep[2],&sz),mymap, CPT);
					CPT++;
